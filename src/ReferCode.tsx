...
export default function ReferCode({ code }: props) {
    ...
    return (
        <View style={[tw`border-2 rounded-sm border-dashed border-emerald-700 p-2  w-full items-center`]}>
            <Pressable style={[tw`absolute right-1 top-1`]} onPress={() => copyToClipboard()}>
                <Image resizeMode='contain' source={require('../assets/copy.png')} />
            </Pressable>
            <Text numberOfLines={1} style={[tw`text-2xl font-bold text-emerald-700 uppercase`]}>{code}</Text>
        </View>
    )
}